...
### (1) Steps to create a distributed application with RPC:
...
2) Use rpcgen with the IDL file:
   ...
   * This command should generate the following files:
     ...
     | message_clnt.c    | No     | Client             | RPC stub or proxy on the client side                                       |
     | message_svc.c     | No     | Server             | RPC stub or proxy on the server side                                       |
     | message_server.c  | Yes    | Server             | Skeleton for implementing the interface on the server. Used as a template for lib-server.c  |
     | message_client.c  | Yes    | Client             | Example client program that makes RPC calls                                |
     | Makefile.message  | Yes    | For compiling      | Template for compilation file.<br>You must add the extra project files and check that the options are appropriate for the project  |
